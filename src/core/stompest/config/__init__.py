...
class SSLConfig(object):
    def __init__(self, sslVersion=None, certReqs=None, options=None, ciphers=None):
        """
        As a general rule, leave all as default.
        Only change if you know what you're doing.

        :param sslVersion: Valid inputs: ssl.PROTOCOL_SSLv2,
                                          ssl.PROTOCOL_SSLv23,
                                          ssl.PROTOCOL_SSLv3,
                                          ssl.PROTOCOL_TLSv1,
        ...
        :type sslVersion: int
        :param certReqs: Valid inputs are: ssl.CERT_NONE, ssl.CERT_OPTIONAL, ssl.CERT_REQUIRED
        ...
        self.certReqs = ssl.CERT_REQUIRED if certReqs is None else certReqs
    ...
    def context(self):
        ...
        if self.sslVersion is not None:
            context.protocol = self.sslVersion
        if self.options is not None:
            context.options |= self.options
        if self.ciphers is not None:
            if getattr(context, 'supports_set_ciphers', True):  # Platform-specific: Python 2.6
                context.set_ciphers(self.ciphers)
        context.verify_mode = self.certReqs
...
class StompConfig(object):
    ...
    def __init__(self, uri, login=None, passcode=None, version=None, check=True, sslConfig=None):
        ...
        self.check = check
        self.ssl_config = sslConfig if sslConfig is not None else SSLConfig()
